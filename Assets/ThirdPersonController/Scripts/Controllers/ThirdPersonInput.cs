...
namespace CoverShooter
{
    ...
    public class ThirdPersonInput : MonoBehaviour
    {
        ...
        protected virtual void UpdateMovement()
        {
            ...
            {
                if (_motor.ActiveWeapon.Gun != null && FastMovement)
                {
        ...
        protected virtual void UpdateClimbing()
        {
            if (Input.GetButtonDown("Climb"))
            {
                ...
                if (direction.magnitude > float.Epsilon)
                {
                    ...
                    if (cover != null)
                        _motor.InputClimbOrVault(cover);
                }
            }
        }
        ...
        protected virtual void UpdateJumping()
        {
            if (Input.GetButtonDown("Jump"))
            {
                ...
                if (direction.magnitude > float.Epsilon)
                    _motor.InputJump(Util.HorizontalAngle(direction) + aimAngle, 1);
                else
        ...
        private int currentWeapon
        {
            get
            {
                ...
                for (int i = 0; i < _inventory.Weapons.Length; i++)
                    if (_inventory.Weapons[i] == _motor.Weapon)
                        return i + 1;
        ...
        private void inputWeapon(int index)
        {
            ...
            {
                _motor.Weapon = _inventory.Weapons[index - 1];
        ...
        private void roll(Vector3 local)
        {
            ...
            if (direction.sqrMagnitude > float.Epsilon)
                _motor.InputRoll(Util.HorizontalAngle(direction));
        }
        ...
        private Vector3 getMovementDirection(Vector3 local)
        {
            ...
            Util.Lerp(ref _leftMoveIntensity, _motor.IsFree(-right) ? 1.0f : 0.0f, 4);
            Util.Lerp(ref _rightMoveIntensity, _motor.IsFree(right) ? 1.0f : 0.0f, 4);
            Util.Lerp(ref _backMoveIntensity, _motor.IsFree(-forward) ? 1.0f : 0.0f, 4);
            Util.Lerp(ref _frontMoveIntensity, _motor.IsFree(forward) ? 1.0f : 0.0f, 4);
