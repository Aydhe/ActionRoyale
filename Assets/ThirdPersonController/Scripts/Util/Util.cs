...
namespace CoverShooter
{
    ...
    public sealed class Util
    {
        ...
        {
            var count = Physics.OverlapSphereNonAlloc(position, 1, Colliders, 0x1 << Layers.Zones, QueryTriggerInteraction.Collide);
            ...
            {
                {
                    var zone = DarkZones.Get(Colliders[i].gameObject);
                    if (zone != null) _dark.Add(zone);
                }
                {
                    var zone = LightZones.Get(Colliders[i].gameObject);
                    if (zone != null) _light.Add(zone);
                }
                {
                    var zone = GrassZones.Get(Colliders[i].gameObject);
        ...
        public static bool IsFree(GameObject gameObject, Vector3 origin, Vector3 direction, float distance, bool coverMeansFree = false)
        {
        ...
        {
            var minDistance = 0f;
            var count = Physics.OverlapSphereNonAlloc(position, radius, Colliders, 0x1 << Layers.Cover, QueryTriggerInteraction.Collide);
        ...
        public static Vector3 GetClosestStaticHit(Vector3 origin, Vector3 target, float minDistance, GameObject ignore = null)
        {
            ...
            var maxDistance = Vector3.Distance(origin, target);
            var closestHit = target;
            var count = Physics.RaycastNonAlloc(origin, vector, Hits, maxDistance, 0x1, QueryTriggerInteraction.Ignore);
        ...
        public static float RandomUnobstructedAngle(GameObject gameObject, Vector3 position, float current, float angleRange, float maxObstruction, int attempts = 10)
        {
            ...
            while (attemptsLeft > 0)
            {
                ...
                if (IsFree(gameObject, position, vector, maxObstruction))
                    return angle;
