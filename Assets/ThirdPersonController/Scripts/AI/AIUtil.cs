...
namespace CoverShooter
{
    ...
    public static class AIUtil
    {
        ...
        {
            int count = 0;
            var physicsCount = Physics.OverlapSphereNonAlloc(position, radius, _colliders, 0x1 << Layers.Character);
        ...
        {
            RaycastHit hit;
            if (Physics.Raycast(origin, (target - origin).normalized, out hit, Vector3.Distance(origin, target), 0x1, QueryTriggerInteraction.Ignore))
            {
