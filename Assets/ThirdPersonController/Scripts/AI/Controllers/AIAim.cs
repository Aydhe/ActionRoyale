    ...
    public class AIAim : AIBase
    {
        ...
        private void turn(ref Vector3 current, Vector3 target, bool isSlow, float multiplier = 1.0f)
        {
            ...
            else if (_aimDelay <= float.Epsilon)
            {
                ...
                var currentAngle = Util.HorizontalAngle(currentVector);
                var targetAngle = Util.HorizontalAngle(targetVector);
                ...
                var length = Util.Lerp(currentVector.magnitude, targetVector.magnitude, speed);

                var position = transform.position + Util.HorizontalVector(currentAngle) * length;
                position.y = Util.Lerp(current.y, target.y, speed);
