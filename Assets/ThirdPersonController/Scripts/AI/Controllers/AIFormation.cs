...
namespace CoverShooter
{
    ...
    public class AIFormation : AIBase
    {
        /// <summary>
        ...
        private void Update()
        {
            ...
            if (this == Leader)
            {
                    ...
                    case FormationType.Square:
                        {
                            ...
                            recursiveEvaluate(0, ref minDist);
                        } break;
                }
                ...
                for (int i = 0; i < _followers.Count - 1; i++)
                    ...
                    {
                        if (Formation == FormationType.Free)
                        {
        ...
        private void recursiveEvaluate(int index, ref float minDist)
        {
            for (int i = 0; i < _followers.Count; i++)
                if (!containsInPositionBuild(index, i))
                {
                    ...
                    if (index + 1 == _followers.Count)
                        evaluatePositionBuild(ref minDist);
                    else
                        recursiveEvaluate(index + 1, ref minDist);
                }
        }
        ...
        private void evaluatePositionBuild(ref float minDist)
        {
            var dist = 0f;

            for (int id = 0; id < _followers.Count; id++)
                dist += Vector3.Distance(_followers[id].transform.position, followerPositionAt(_positionBuild[id]));
        ...
        private Vector3 followerPositionFor(int index)
        {
            if (_followerToPositionId.Count > index)
                return followerPositionAt(_followerToPositionId[index]);
            else
            {
                Debug.Assert(false);
                return followerPositionAt(index);
            }
        }
        ...
        private Vector3 followerPositionAt(int index)
        {
            switch (Formation)
            {
                case FormationType.Square:
        ...
        private void updatePositional()
        {
            ...
            var target = Leader.followerPositionFor(_registrationId);
            ...
            if (hasToReposition)
            {
                ...
                if (hasToMoveAlong)
                {
                        ...
                        (distance > FreeRoamInDanger.FollowDistance && shift.magnitude > 0.1f))
                    {
                        ...
                        if (!Util.IsFree(gameObject, transform.position + Vector3.up * 0.25f, direction, 2))
